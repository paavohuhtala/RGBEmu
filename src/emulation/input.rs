...
impl JoypadRegister {
  fn from_input_state(state: InputState, selected_set: ButtonSet) -> JoypadRegister {
    ...
    match selected_set {
      ButtonSet::Arrows => {
        if state.up { base.remove(JoypadRegister::UpOrSelect); }
        if state.down { base.remove(JoypadRegister::DownOrStart); }
        if state.left { base.remove(JoypadRegister::LeftOrB); }
        if state.right { base.remove(JoypadRegister::RightOrA); }
      },
      ButtonSet::Actions => {
        if state.a { base.remove(JoypadRegister::RightOrA); }
        if state.b { base.remove(JoypadRegister::LeftOrB); }
        if state.select { base.remove(JoypadRegister::UpOrSelect); }
        if state.start { base.remove(JoypadRegister::DownOrStart); }
      },
      ButtonSet::None => ()
    }
