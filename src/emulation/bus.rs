...
use crate::emulation::interrupt::{Interrupt, InterruptRegisters};
...
impl Bus {
    ...
    pub fn read_to_buffer(&self, buffer: &mut [u8], address: u16, length: u16) {
        if buffer.len() < (length as usize) {
            panic!(
                "Tried to read {} bytes to a buffer sized {}.",
                buffer.len(),
                length
            );
        }
...
impl AddressMapper for Bus {
    ...
    fn read_8(&self, location: MemoryLocation) -> u8 {
        ...
        match location {
            ...
            RamBankN(offset) => {
                self.ram[self.selected_ram_bank * RAM_BANK_SIZE + (offset as usize)]
            }
            Cartridge(address) => {
        ...
        match location {
            ...
            _ => {
                match location {
                    ...
                    RamBankN(offset) => {
                        self.ram[self.selected_ram_bank * RAM_BANK_SIZE + (offset as usize)] = value
                    }
                    Cartridge(address) => match self.cartridge {
