...
impl Mapper for RomOnly {
    ...
    fn read_8(&self, memory: &CartridgeMemory, address: u16) -> u8 {
        ...
        match location {
            ...
            Ram(_) => {
                println!(
                    "Tried to read from cartridge RAM in a cart without a mapper! (0x{:04X})",
                    address
                );
                0
            }
    ...
    fn write_8(&mut self, _cart: &mut CartridgeMemory, address: u16, value: u8) {
        match address {
            _ => println!(
                "Tried to write to a cartridge without a mapper! ({:?} <- {:X})",
                address, value
            )
        }
    }
...
impl dyn Mapper {
    pub fn from_cartridge_type(cartridge_type: CartridgeType) -> Box<dyn Mapper> {
        match cartridge_type.mapper {
            ...
            _ => panic!(
                "Tried to create create an unsupported mapper: {:?}",
                cartridge_type.mapper
            )
        }
    }
}
...
impl MBC1 {
    ...
    fn get_ram_bank(&self) -> u8 {
        if self.rom_banking_mode {
            ...
            self.ram_bank_or_upper_rom_bits & 0b11
        }
    }
    ...
    fn resolve_address(&self, address: u16) -> MBC1Location {
        ...
        match address {
            ...
            ROM_BANK_N_START..=ROM_BANK_N_END => RomBankN(
                rom_bank as u32 * ROM_BANK_SIZE as u32 + (address - ROM_BANK_N_START) as u32
            ),
            CARTRIDGE_RAM_START..=CARTRIDGE_RAM_END => {
                Ram(ram_bank as u32 * RAM_BANK_SIZE as u32 + (address - CARTRIDGE_RAM_START) as u32)
            }
